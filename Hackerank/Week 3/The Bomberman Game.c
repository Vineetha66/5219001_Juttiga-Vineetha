 ...
 *
 */
char** mfg(int a, int b, int* rc){
    char** f = malloc(a * sizeof(char*));
    for(int i = 0; i < a; i++){
        f[i] = malloc(b + 1);
        for(int j = 0; j < b; j++){
            f[i][j] = 'O';   // bombs must be uppercase O
        }
        f[i][b] = '\0';
    }
    *rc = a;
...
char** e(int a, int b, char** g, int* rc){
    char** ng = malloc(a * sizeof(char*));
    for(int i = 0; i < a; i++){
        ng[i] = malloc(b + 1);
        for(int j = 0; j < b; j++){
            ng[i][j] = 'O';   // start with full bombs
        }
        ng[i][b] = '\0';
    }
    ...
    for(int i = 0; i < a; i++){
        for(int j = 0; j < b; j++){
            if(g[i][j] == 'O'){   // must check uppercase O
                for(int k = 0; k < 5; k++){
                    int n1 = i + x[k];
                    int n2 = j + y[k];
                    if(n1 >= 0 && n1 < a && n2 >= 0 && n2 < b){
                        ng[n1][n2] = '.';  // explosion clears
                    }
                }
            }
        }
...
char** bomberMan(int n, int grid_count, char** grid, int* result_count) {
    ...
    if(n%2==0){
        return mfg(a,b, result_count);
    }
    char** a1 = e(a, b, grid, result_count);
    
    if(n%4==3){
        return a1;
    }
    char** a2 = e(a,b, a1, result_count);
    return a2;
}
